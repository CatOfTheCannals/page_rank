...
% una breve explicación de los motivos de estas fallas (en caso de ser conocidas).
	...
	\subsection{Elección de la estructura de datos}
		...
		En particular, vamos a querer multiplicar a sus elementos \textit{distintos de $0$} (pues los que son $0$ no haría falta porque quedarían iguales), y la matriz $W$, por ser rala, contiene muchos elementos iguales a $0$. Es por esto que decidimos usar un \textit{map$<$int, double$>$} para almacenar las columnas: donde el \textit{int} es el número de fila y el \textit{double} es el valor almacenado en esa posición del \textit{map} (es decir, de la columna). \\
		...
		$E_{n,n-1} \hdots E_{2,1} (I - pWD) x = E_{n,n-1} \hdots E_{2,1} e$ \\

		Esto, a su vez, es equivalente a hacer operaciones con entre las filas. Por ejemplo, si $E_{2,1}$ tiene un escalar $m_{2,1} \neq 0$ en la posición $2,1$, premultiplicar $E_{2,1}$ por $(I - pWD)$ será modificar $(I - pWD)$ de manera tal que en su segunda fila
